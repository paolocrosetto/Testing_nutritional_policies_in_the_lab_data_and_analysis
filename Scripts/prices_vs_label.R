...
# getting the data
cp <- df %>% 
  filter(treatment %in% c("NS", "pc_exp"))
...
sink(file = "Tables/tab_label_vs_prices.tex")
nice_table %>% 
...
ggsave("Figures/prices_vs_label_presentation.png", width = 12, height = 5, units = "in", dpi = 300)
...
cp %>% 
  ...
  theme(panel.grid.minor = element_blank(),
        legend.position = "none")+
  labs(x = "", y = "")
ggsave("Figures/label_vs_price.png", width = 16/1.4, height = 8/1.4, units = "in", dpi = 300)

## testing one against the other
tests <- cp %>% 
  group_by(indicator) %>% 
  group_modify(~tidy(wilcox.test(.$diff[.$treatment == "NutriScore"], .$diff[.$treatment == "Explicit price"]))) %>% 
  mutate(p.value = round(p.value, 3)) %>% 
  select(indicator, p.value)

sink("Tables/tests_label_price.tex")
tests %>% 
  kbl(booktabs = T, 
      #col.names = c("", rep(c("cart 1", "cart 2", "difference", "p-value"),2)), 
      format = "latex", 
